...
def build_schedule_report_envelope(
    ...
    scheduleReport operation. Optional string fields are omitted when empty.
    The ``parameters`` dict maps parameter names to their values; ``None``
    values are skipped entirely; scalar values produce one ``<v2:item>``
    element; list values produce one ``<v2:item>`` element per entry.
    ...
    # --- credentials (same level as reportRequest / scheduleRequest) ---
